...
def combine_lrc_images(first_src, second_src, dest_file):
    ...
    left_combined = np.append(first_imgs[third_length_of_first: 2 * third_length_of_first],
                              second_imgs[third_length_of_second: 2 * third_length_of_second], axis=0)
    right_combined = np.append(first_imgs[2 * third_length_of_first: length_of_first],
                               second_imgs[2 * third_length_of_second: length_of_second], axis=0)
...
def combine_all(np_dir, img_prefix, dest_name, length):
    ...
    for i in range(6000, length, 2000):
        ...
        combine_images(np_dir + dest_name, np_dir + str(last_start) + '_' + str(i) + '_' + img_prefix,
                       np_dir + dest_name)
        last_start = i
    ...
    if (length - last_start != 2000):
        ...
        combine_images(np_dir + dest_name, np_dir + str(last_start) + '_' + str(length) + '_' + img_prefix,
                       np_dir + dest_name)
    print('combined all')
...
def combine_lrc_labels(first_src, second_src, dest_file):
    ...
    center_combined = np.append(first_labels[0: third_length_of_first], second_labels[0: third_length_of_second],
                                axis=0)
    left_combined = np.append(first_labels[third_length_of_first: 2 * third_length_of_first],
                              second_labels[third_length_of_second: 2 * third_length_of_second], axis=0)
    right_combined = np.append(first_labels[2 * third_length_of_first: length_of_first],
                               second_labels[2 * third_length_of_second: length_of_second], axis=0)
...
def resize_all(src_file, np_dir, dest_name, width, height):
    ...
    for i in range(2000, end, 2000):
        ...
        resize_file_images(src_file, np_dir + str(last_start) + '_' + str(i) + '_' + dest_name, width, height,
                           last_start, i)
        last_start = i
    ...
    if (end - last_start != 2000):
        ...
        resize_file_images(src_file, np_dir + str(last_start) + '_' + str(end) + '_' + dest_name, width, height,
                           last_start, end)
    print('resized all')
...
if __name__ == '__main__':
    img_dir = './training/t2/IMG'
    csv_dir = './training/t2/driving_log.csv'
    np_dir = './training/np_data/'
    ...
    # 2b. Crop images to height: 60 - 130
    # Combine all
    # combine_all(np_dir,)
